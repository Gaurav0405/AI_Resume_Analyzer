...
connection = pymysql.connect(host='localhost',user='root',password='Mohit7415',db='cv')
...
def get_user_leaderboard(reco_field):
    valid_fields = ["Predicted_Field", "User_level", "Actual_skills", "Recommended_skills"]
    if reco_field not in valid_fields:
        raise ValueError("Invalid reco_field")

    leaderboard_sql = f"""
    SELECT 
        Name,
        CAST(resume_score AS UNSIGNED) AS res_score,
        RANK() OVER (ORDER BY CAST(resume_score AS UNSIGNED) DESC) AS `rank`,
        ROUND(
            (SELECT COUNT(*) FROM user_data u2 
             WHERE CAST(u2.resume_score AS UNSIGNED) < CAST(u1.resume_score AS UNSIGNED)) * 100.0 /
            (SELECT COUNT(*) FROM user_data u2), 
            2
        ) AS better_than_percent
    FROM user_data u1
...
def run():
    ...
    link = '<b>Built with 🤍 by Group 72</b>' 
    ...
    st.sidebar.markdown('''
        ...
        <noscript>
           <a href="https://www.freecounterstat.com" title="website counter"><img src="https://counter4.optistats.ovh/private/freecounterstat.php?c=jqwh82e52fkfpcudeunwfar1akuuk8sh" border="0" title="website counter" alt="website counter"></a>
        </noscript>
    
        <p>Visitors <img src="https://counter4.optistats.ovh/private/freecounterstat.php?c=jqwh82e52fkfpcudeunwfar1akuuk8sh"  title="Free Counter" Alt="web counter" width="60px"  border="0" /></p>
    ...
    if choice == 'User':
        ...
        if pdf_file is not None:
            ...
            resume_data = resume_parser(save_image_path).get_extracted_data()
            if resume_data:
                ...
                web_keyword = ['html','css','react', 'django', 'node jS', 'react js', 'php', 'laravel', 'magento', 'wordpress','javascript', 'angular js', 'C#', 'Asp.net', 'flask']
                ...
                st.subheader("📊 Leaderboard in Your Domain")

                leaderboard = get_user_leaderboard(reco_field)

                if leaderboard:
                    leaderboard_df = pd.DataFrame(leaderboard, columns=[
                        "Name", "Score", "Rank", "Better Than (%)"
                    ])

                    st.dataframe(leaderboard_df.style.highlight_max(axis=0, color='lightgreen'))

                    # Find current user's "better than" percentage
                    for row in leaderboard:
                        if row[0] == act_name and float(row[1]) == float(resume_score):  # Matching by name & score
                            st.success(
                                f"🎉 You are better than {row[3]}% of the people who submitted resumes in the same domain!")
                            break
                else:
                    st.info("Not enough data yet to show leaderboard.")
        ...
        <p align="justify">
            ...
            <b>Admin -</b> <br/>
            For login use <b>admin</b> as username and <b>vit</b> as password.<br/>
            It will load all the required stuffs and perform analysis.
        </p><br/><br/>

        <p align="justify">
            Built with 🤍 by 
            <a style="text-decoration: none; color: grey;">Group 72</a> 
        </p>
        ...
        if st.button('Login'):
            ...
            if ad_user == 'admin' and ad_password == 'vit':
                ...
                st.success("Welcome Mohit ! Total %d " % values + " User's Have Used Our Tool : )")                
